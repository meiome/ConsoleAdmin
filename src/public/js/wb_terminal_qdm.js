	...
	xhttp.onreadystatechange = function() {
		...
		xelemento = null;
		 for (var i in this.response.data) {
			 ...
			 //in questo modo prendo l'ultimo... per ora non gestita più di un ciclo
			 //questi sotto mi servono solo per creare la stringa per reindirizzare nel caso di scelta inserimento

			 //xmethod = this.response.data[i].query.method;
			 //xid = this.response.data[i].query.id;
			 xelemento = this.response.data[i].model.elemento;
			 xelementoname = this.response.data[i].model.name;
			 //console.log(xid);
			 //console.log(xelemento);
			 //console.log(xelementoname);
		 }

		 if (xelemento !== null){
			 xmethod = 'prepareinsert';
			 xid = '0';
			 prepareinsertlocation = "/terminal/"+xelemento+"/"+xmethod+"/"+xelementoname+"/"+xid;
		 }
	...
	xhttp.onreadystatechange = function() {
		...
		xid = 0;
		 for (var i in this.response.data) {
			 qmd.push(this.response.data[i]);//possono esserci più di una istruzione contemporanea (esempio delete di più elementi selezionati da checkbox)
			 //in questo modo prendo l'ultimo... per ora non gestita più di un ciclo
			 //questi sotto mi servono solo per creare la stringa per reindirizzare dopo il salvataggio all'elemento appena salvato
			 xmethod = this.response.data[i].query.method;
			 xid = this.response.data[i].query.id;
			 xelemento = this.response.data[i].model.elemento;
			 xelementoname = this.response.data[i].model.name;
		 ...
		 //window.location.href = "/terminal/maschera/prepareinsert/articoli/0";
		 if (xid > 0){
			 xlocation = "/terminal/"+xelemento+"/"+xmethod+"/"+xelementoname+"/"+xid;
...
/*function terminalgestoredataforserviziobyquery(time,xarray,url,urledit){
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		//	this.qmd = this.response.data;
		 //console.log(this.response.data[0].data.fdr);
		 for (var i in this.response.data[0].data.fdr) {

			 //uqmd.push(this.response.data[0].data.fdr[i]);//possono esserci più di una istruzione contemporanea (esempio delete di più
			 caricaservizio(this.response.data[0],this.response.data[0].data.fdr[i],i,url,urledit);
		 }
		 aggiungiriga("## "+this.response.data[0].data.fdr.length+" elementi servizio caricati ##","", false, false);
		 aggiungiriga("## inizio esecuzione ##","", false, false);
		 aggiungiriga("","", false, false);
		 lanciaservizio(time);
		}
	};
	xhttp.open("POST", "/terminal/gestoredatiwithmodel", true);
	xhttp.responseType = 'json';
	xhttp.setRequestHeader("Content-Type", "application/json;charset=utf-8");
	xhttp.send(getparameterreqforterminal(xarray));
}*/

function terminalgestoredataforserviziolist(time,xarray,url,urledit){
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		//	this.qmd = this.response.data;
...
function terminalgestoredataforservizio(time,qmd,id,cont,url,urledit){
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		//	this.qmd = this.response.data;
		if (this.response.success) {
			cont++;//aggiungo 1 perchè il contatore parte da 0
			aggiungitestoultimariga(" #");
			//aggiungiriga("# "+cont+"(id"+id+")-> "+this.response.message+" "+this.response.code+" "+this.response.success,"",false,false,false);
			//aggiungiriga("##########################","width:100%; max-height: 3px; background-color: #2e2e2e;",false,false,false);
		} else {
			var a = document.createElement("a");
			var modelname = "article"
      a.setAttribute("href", urledit+id);/*come prop usa l'ultima usata tanto hanno tutte l'id*/
      a.innerText = '🔗';

			cont++;//aggiungo 1 perchè il contatore parte da 0
			aggiungiriga("# "+cont+"(id"+id+")-> "+this.response.message+" "+this.response.code+" "+this.response.success,"color:red;",false,false,a);
		}
		lanciaservizio(time);
		}
	};
...
function terminalgestoredataforlista(xarray){
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		//	this.qmd = this.response.data;
		 for (var i in this.response.data) {
			 tqmd.push(this.response.data[i]);//possono esserci più di una istruzione contemporanea (esempio delete di più elementi selezionati da checkbox)
			 stampalista(this.response.data[i]);
		 }
		}
	};
	xhttp.open("POST", "/terminal/gestoredatiwithmodel", true);
...
function getparameterreqforterminal(xarray){

	this.reqtqmd = [];//lo svuoto per salvare i nuovi dati

		maschera = {};
		model = {};
		model.name = xarray[1];
		model.type = 'main'
		model.elemento = 'maschera';
		data = {};
		query = {};
		query.method = xarray[0];//passo il metodo esempio lista (diventato "list")
		query.id = "0";//questo è zero per default ??? valutare come sistemare
		query.arraycommand = xarray;

		maschera.data = data;
		maschera.model = model;
		maschera.query = query;

		this.reqtqmd.push(maschera);

/*		for (index = 0; index < reqtqmd.length; ++index) {
			var ob = reqtqmd[index].query;
			ob.method = this.reqqmdmethod;
		}*/
	return JSON.stringify(reqtqmd);
}

var tlmqmd = [];

function terminalgestoredataforlistamascheredisponibili(){//mostro a terminale le maschere per le quali sono abilitato
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		//	this.qmd = this.response.data;
		stampalistamaschere(this.response.data);
		}
	};
	xhttp.open("POST", "/terminal/listamaschere", true);
	xhttp.responseType = 'json';
	xhttp.setRequestHeader("Content-Type", "application/json;charset=utf-8");
	xhttp.send();
}
...
function editmaschera(){
	...
	inputs = document.getElementById('maincontenitor').getElementsByTagName('textarea');
	for (index = 0; index < inputs.length; ++index) {
		 inputs[index].disabled = false;
	}
	selects = document.getElementById('maincontenitor').getElementsByTagName('select');
...
var prepareinsertlocation = '/terminal';

function nuovamaschera(){
	console.log("premuto tasto Nuova maschera MAIN");
	mostrafinestraattesa();

	//console.log(prepareinsertlocation);
	window.location.href = prepareinsertlocation;
	//window.location.href = "/terminal/maschera/prepareinsert/articoli/0";
...
function checknullvalueecoerenza(){
	...
	for (index = 0; index < inputs.length; ++index) {
		...
		campook = true;
		if(inputs[index].getAttribute("lideleted") == null){//qui vado a testare se quel campo appartiene ad un li cancellato (basta che sia presente l'attributo lideleted) in quel caso non serve testarlo
			if((inputs[index].getAttribute("campo")).localeCompare("DATE") == 0){//se è un input con dati tipo date
				if(inputs[index].getAttribute("isnullable") == 'false' && (inputs[index].value).localeCompare("") == 0){
				//se non è nullable e è vuoto do errore
						campook = false;
					...
					if ((inputs[index].value).localeCompare("") != 0){
						...
						if ( parola.length != 10
							...
							|| parseInt(parola.substring(0, 4)) < 2010 //anno troppo vecchio
					...
					if((inputs[index].getAttribute("campo")).localeCompare("DATETIME") == 0){//se è un input con dati tipo date
						if(inputs[index].getAttribute("isnullable") == 'false' && (inputs[index].value).localeCompare("") == 0){
							//se non è nullable e è vuoto do errore (aggiungo il messaggio effimero e metto il flag campook a false)
								campook = false;
								AggiungiMessaggioEffimero("DataOra Non Valorizzata",inputs[index].getAttribute("camponame"));
						} else {
							if ((inputs[index].value).localeCompare("") != 0){
								//se il campo è valorizzato indipendentemente dal nullable controllo coerenza
								parola = inputs[index].value; //es 2023-02-08T14:37 mozzilla //chroome 2023-02-08T14:47
								...
								if ( parola.length < 16
									...
									campook = false;
									AggiungiMessaggioEffimero("DataOra non coerente",inputs[index].getAttribute("camponame"));
								}
								/* NON SERVONO PIù questi controlli che ho deciso di non mettere parseInt()
								parola.substring(0, 4);//anno es 2021
								parola.substring(4, 5);//trattino divisorio - tra anno e mese
					...
					if((inputs[index].getAttribute("campo")).localeCompare("DOUBLE") == 0){//se è un DOUBLE
						...
						if((inputs[index].getAttribute("campo")).localeCompare("INT") == 0){//se è un interval
							//se non è nullable e è vuoto do errore
							if(inputs[index].getAttribute("isnullable") == 'false' && (inputs[index].value).localeCompare("") == 0){
									campook = false;
									AggiungiMessaggioEffimero("Campo Numerico Intero Obbligatorio Non Valorizzato",inputs[index].getAttribute("camponame"));
							} else {
								if ((inputs[index].value).localeCompare("") != 0){
									//se il campo è valorizzato indipendentemente dal nullable controllo coerenza
									parola = inputs[index].value;

									if (parola.indexOf(' ') >= 0 || parola.match('[^0-9]') /*se contiene qualcosa che non è numeri 0-9*/|| Number.isNaN(parseInt(parola))){
										campook = false;
										AggiungiMessaggioEffimero("Campo Numerico Intero non coerente",inputs[index].getAttribute("camponame"));
									}
								}
							}
						} else {
	...
	for (index = 0; index < inputs.length; ++index) {
		...
		if(inputs[index].getAttribute("isnullable") == 'false'){
			if((inputs[index].value).localeCompare("") == 0){
				campook = false;
				AggiungiMessaggioEffimero("Text area non Valorizzata",inputs[index].getAttribute("camponame"));
			}
		}
		if (campook == false){
			//se sono state trovate incoerenze evidenzio l'errore
	...
	//fine ciclo su tutte le TEXTAREA

	//ciclo sullo SELECT <OPTION>
	selects = document.getElementById('maincontenitor').getElementsByTagName('select');
	for (index = 0; index < selects.length; ++index) {
		if(selects[index].getAttribute("lideleted") == null){//qui vado a testare se quel campo appartiene ad un li cancellato (basta che sia presente l'attributo lideleted) in quel caso non serve testarlo
...
function salvamaschera(){
	...
	if (test==false){//inizio IF checknullvalueecoerenza()
		...
		for (index = 0; index < inputs.length; ++index) {
			if((inputs[index].getAttribute("campo")).localeCompare("DATE") == 0){//se è un input con dati tipo date
				...
				if((inputs[index].getAttribute("campo")).localeCompare("TINYINT(1)") == 0){
					...
					if(inputs[index].getAttribute("iscampofile").localeCompare("true") == 0){//campo input tipo file

						if (inputs[index].getAttribute("lideleted") == 1){
							//caso in cui bisogna cancellare il file se era presente a db il nome files
							if ((inputs[index].getAttribute("dbvalue")).localeCompare('')==0) {

							} else {
								var xmaschname = this.qmd[0].model.name;//'articoli';
								var xcartella = this.qmd[0].model.cartella;//'articoli';

								xfilenametodelete = inputs[index].getAttribute("dbvalue");

								eliminafile(xmaschname,xcartella,xfilenametodelete);
							}
						} else {

							
							//caso in cui la riga non viene eliminata-> solo in questo caso carico il file
							if (inputs[index].files[0] !== undefined) {//se è selezionato almeno un file

								var xmaschname = this.qmd[0].model.name;//'articoli';
								var xcartella = this.qmd[0].model.cartella;//'articoli';

								if ((inputs[index].getAttribute("dbvalue")).localeCompare('')==0) {//controllo se è già presente un file salvato per quell'elemento (controllo il dbvalue e non isnewelement perchè può essere che sia già salvato senza file)
									//nuovo elemento file mai aggiunto prima
									xfilename = randomfilename(30);
									arrayfilename = (inputs[index].files[0].name).split(".");//trovo estensione file
									xfileext = arrayfilename[arrayfilename.length - 1];
									...
									//qui devo anche cancellare il vecchio file
									//non posso sovrascriverlo perchè può avere estensione diversa
									xfilenametodelete = inputs[index].getAttribute("dbvalue");
									...
									xhttp.open("POST", "/terminal/deletefile/"+xmaschname+"/"+xcartella+"/"+xfilenametodelete);
									xhttp.responseType = 'json';
									xhttp.send();*/
								}
								...
								var file = inputs[index].files[0];
								xhttp.open("POST", "/terminal/gestorefile/"+xmaschname+"/"+xcartella+"/"+xfilename);
								xhttp.responseType = 'json';

								//xhttp.setRequestHeader('Content-Type',"multipart/form-data");
								xhttp.setRequestHeader('X-File-Name',file.fileName);
								xhttp.setRequestHeader('X-File-Size',file.fileSize);
								xhttp.setRequestHeader('X-File-Type',file.fileType);
								xhttp.send(file);

								//setDataQmd(inputs[index],inputs[index].name);//passo il file come valore

								//nel caso in cui sia jpg uso lo stesso estensione jpeg perchè lato server mette quella sicuramente
								if (xfileext.localeCompare("jpg")==0){
									xfileext="jpeg";
								}

								setDataQmd(inputs[index],xfilename+'.'+xfileext);
							}
						}
					} else {
						//tutti gli altri
						if((inputs[index].value).localeCompare(inputs[index].getAttribute("dbvalue"))) {
							...
							setDataQmd(inputs[index],(inputs[index].value).replace("'","''"));
						}
					}
				}
			}
		...
		for (index = 0; index < inputs.length; ++index) {
			if((inputs[index].value).localeCompare(inputs[index].getAttribute("dbvalue"))) {
				console.log('salvamaschera()textarea/'+inputs[index].value+'/');
				//console.log(inputs[index].value);
				//console.log((inputs[index].value).replace("'","''"));
				setDataQmd(inputs[index],(inputs[index].value).replace("'","''"));
			}
		}
...
function eliminafile(xmaschname,xcartella,xfilenametodelete){
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/terminal/deletefile/"+xmaschname+"/"+xcartella+"/"+xfilenametodelete);
	xhttp.responseType = 'json';
	xhttp.send();
...
function randomfilename(length = 30) {
		if ( length < 30 ) {
			length = 30;
		}
    let result = '';
...
function setDataQmdRimuoviElemento(indexqmd,indexfdr,indexcampi,indexfdrsub,indexcampisub,camponame,mascherasub,value,li){
	...
	if (mascherasub){
		...
		li.style.display = 'none';
			...
			//nuova modifica per escludere righe cancellate non valorizzate correttamente dal checknullvalueecoerenza, altrimenti questo da errore al salvataggio se non sono valorizzati bene tutti i campi
			inputs = li.getElementsByTagName('input');
			for (index = 0; index < inputs.length; ++index) {
				//INIZIO CICLO SU TUTTI GLI INPUT
				inputs[index].setAttribute("lideleted", "1");//ho messo 1 ma potevo mettere qualsiasi cosa perchè poi testerò solo la presenza dell'attributo
			}

			selects = li.getElementsByTagName('select');
			for (index = 0; index < selects.length; ++index) {
				selects[index].setAttribute("lideleted", "1");//ho messo 1 ma potevo mettere qualsiasi cosa perchè poi testerò solo la presenza dell'attributo
			}
			//stop nuova modifica per escludere righe cancellate non valorizzate correttamente
...
function getMascheraLi(isnewelement,elementpadreappend,indexqmd,Xindexfdrmain,indexcampimain,Xindexfdrsub,data,mascherasub,query){
	contatorecampi = 0;
	cicli = 0;//lo uso per apendere i link lo appendo all'inizio
	//quetso ciclo gira sull'array campi per avere i dati fissi di ogni campo
	for (var c in data.campi) {
		if (mascherasub && cicli<1){
			var datatarget = qmd[indexqmd].data;
			var campotarget = datatarget.campi[indexcampimain];
			if ('linkwithidintoelement' in campotarget){//testo anche se esiste la proprietà che dice di mettere il link

				var camposubtarget = data.campi[c];
				var href = campotarget.linkwithidintoelement+data.fdr[Xindexfdrsub][camposubtarget.name].id;
				...
				divtitle.appendChild(document.createTextNode('link'));
				divcampo.appendChild(divtitle);

				var aelement = document.createElement("a");
				aelement.setAttribute("href",href);
				aelement.appendChild(document.createTextNode('🔗'));
				divcampo.appendChild(aelement);

				elementpadreappend.appendChild(divcampo);
			}
		}
		cicli++;
			...
			if (data.campi[c].type.localeCompare("singlerelation") == 0){
				...
				if (data.campi[c].type.localeCompare("maschera") == 0){
					...
					if (data.campi[c].type.localeCompare("manipolazione") == 0){
						...
						if (data.campi[c].type.localeCompare("separatorlabel") == 0){
							elementpadreappend.appendChild(getHtmlSeparatorLabel(data.campi[c]));
						}
					}
				}
			}
...
function getHtmlSingleRelation(isnewelement,indexqmd,indexfdr,indexcampi,indexfdrsub,indexcampisub,rigafdr,campo,mascherasub,query){
	...
	if (mascherasub){
		...
		divcampo.setAttribute("class","contenitorsinglerelmaschmain");
	}
	...
	if (isnewelement == true){
		...
		if(rigafdr[campo.name].value == null || ("null").localeCompare(rigafdr[campo.name].value) == 0) {
...
function getHtmlSeparatorLabel(campo){
	var divcampo = document.createElement("div");//maschera
	divcampo.setAttribute("class","contenitorcamposeparatorlabel");
	divcampo.appendChild(document.createTextNode(campo.etichetta));
	return divcampo;
}
...
function getHtmlCampo(isnewelement,indexqmd,indexfdr,indexcampi,indexfdrsub,indexcampisub,rigafdr,campo,mascherasub,query){
	...
	if (campo.dbdatatype.localeCompare("LONGTEXT") == 0){
	...
	inp.setAttribute("iscampofile", campo.iscampofile);
	...
	if (campo.dbdatatype.localeCompare("VARCHAR(255)") == 0){
		if (campo.iscampofile) {
			...
			if (isnewelement){
				...
				inp.name = rigafdr[campo.name].value;;//"test";
				if(rigafdr[campo.name].value == null) {
					//nessuna foto già presente
				} else {
					aggiungifoto = true;
				}
			}
			...
			if(campo.dbdatatype.localeCompare("DOUBLE") == 0){
				...
				if(campo.dbdatatype.localeCompare("DATE") == 0){
					...
					if (campo.dbdatatype.localeCompare("LONGTEXT") == 0){
						...
						if(campo.dbdatatype.localeCompare("DATETIME") == 0){
							inp.setAttribute("type", "datetime-local");
							inp.setAttribute("class","inputcampodate");
							if (isnewelement){
								inp.value = "";
							} else {
								//console.log(rigafdr[campo.name].value);
								//value stringa estratta da db mozzilla
								parola = rigafdr[campo.name].value;
								if (parola && parola.length > 10 && parola.substring(10, 11).localeCompare(' ')==0)//se è uno spazio bianco
								{
									//dovrebbe essere sempre uno spazio bianco ho messo l'else solo per casi eccezzionali passa lo stesso il valore
									//console.log(parola.substring(0, 10) + 'T' + parola.substring(11));
									inp.value = parola.substring(0, 10) + 'T' + parola.substring(11);
								} else {
									inp.value = rigafdr[campo.name].value;
								}
							}
						} else {
							if(campo.dbdatatype.localeCompare("INT") == 0){
								inp.setAttribute("type", "text");
								inp.setAttribute("class","inputcampovarchar");
								if (isnewelement){
									inp.value = "";
	...
	if (aggiungifoto) {
		var linkfotoarticolo = document.createElement("a");
		//linkfotoarticolo.setAttribute('href', '/uploads/terminal/articoli/'+rigafdr[campo.name].value);
		//linkfotoarticolo.setAttribute('target','_blank');

		//alert(this.qmd[indexqmd].model.cartella);

		var fotoarticolo = document.createElement("img");
		fotoarticolo.setAttribute('src', '/uploads/terminal/'+this.qmd[indexqmd].model.cartella+'/'+rigafdr[campo.name].value);
		//fotoarticolo.setAttribute('src', '/uploads/terminal/articoli/'+rigafdr[campo.name].value);
		fotoarticolo.setAttribute('class', 'fotocaricata');
		linkfotoarticolo.appendChild(fotoarticolo);
		divcampo.appendChild(linkfotoarticolo);
	}
